...
$lastmonth_row = array();
while ($i < 6) {
    $thismonth_select = "SELECT count(tracking_page_name) FROM hc_track WHERE (tm BETWEEN '" . date('Y-m-01') . "' AND '" . date('Y-m-' . $loop[$i]) . "') AND tracking_page_name = 'homecubic_home'";
    $thismonth_result = $link->query($thismonth_select) or die($link->error);
    $thismonth_row[$i] = $thismonth_result->fetch_array(MYSQLI_BOTH);


    $lastmonth_select = "SELECT count(tracking_page_name) FROM hc_track WHERE (tm BETWEEN '" . date($lastyear . '-' . $lastmonth . '-01') . "' AND '" . date($lastyear . '-' . $lastmonth . '-' . $loop[$i]) . "') AND tracking_page_name = 'homecubic_home'";
