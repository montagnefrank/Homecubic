...
if (isset($_POST['edittitles'])) {
    ...
    $val_select = "UPDATE hc_misc SET solucTitle ='" . $_POST['solucTitle'] . "', solucSubtitle ='" . $_POST['solucSubtitle'] . "' WHERE idSite = '1'";
...
if (isset($_POST['editsoluc'])) {
    ...
    foreach ($loop as $id){
        if (isset($_POST[$id . '_check'])) {
            $val_select = "UPDATE hc_soluc SET titlesoluc ='" . $_POST[$id . '_title'] . "', textSoluc ='" . $_POST[$id . '_text'] . "', statusSoluc ='" . $_POST[$id . '_check'] . "' WHERE idSoluc = '".$id."'";
            $val_result = $link->query($val_select) or die($link->error);
        } else {
            $val_select = "UPDATE hc_soluc SET titleSoluc ='" . $_POST[$id . '_title'] . "', textSoluc ='" . $_POST[$id . '_text'] . "', statusSoluc ='0' WHERE idSoluc = '".$id."'";
            $val_result = $link->query($val_select) or die($link->error);
        }
    }
...
if (isset($_POST['deletesoluc'])) {
    $val_select = "DELETE FROM hc_soluc WHERE idSoluc = '" . $_POST['deleteid'] . "'";
    $val_result = $link->query($val_select) or die($link->error);
...
if (isset($_POST['newsoluc'])) {
    ...
    $val_select = "INSERT INTO hc_soluc(iconSoluc,titleSoluc,textSoluc,statusSoluc) VALUES ('<i class=\"fa " . $_POST['icon'] . "\"></i>','" . $_POST['newsolucTitle'] . "','" . $_POST['newsolucSubtitle'] . "','1')";
    $val_result = $link->query($val_select) or die($link->error);
